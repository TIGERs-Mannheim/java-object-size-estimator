...
final class ObjectProfileNode extends AbstractProfileNode {
  ...
  void addFieldRef(final IObjectProfileNode node) {
    ...
    if (size >= childrenLength) {
      final IObjectProfileNode[] newchildren = new IObjectProfileNode[Math.max(
          1, childrenLength << 1)];
  ...
  /*
   * This method is called once on every node to lock it down into its immutable
   * and most compact representation during phase 2 of profile tree
   * construction.
   */
  void finish() {
    final int childCount = size; // m_size is the child count for a non-shell
                                 // node
    if (childCount > 0) {
      ...
      size = size; // m_size is the full node size for all nodes
    }
  }

  ObjectProfileNode(final ObjectProfileNode parent, final Object obj,
      final ILink link) {
    super(parent);
