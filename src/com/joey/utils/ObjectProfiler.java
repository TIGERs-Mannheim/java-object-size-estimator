 ...
 * as described in the article. See individual methods for details.
 * <P>
 * 
 ...
 * your main application code. The minimum set of persmissions necessary for
...
public abstract class ObjectProfiler {
  ...
  public static final int OBJECT_SHELL_SIZE = 8; // java.lang.Object shell size
                                                 // in bytes
  ...
  /**
   * Set this to 'true' to make the node names default to using class names
   * without package prefixing for more compact dumps
   */
  public static final boolean SHORT_TYPE_NAMES = false;

  /**
   * If this is 'true', node names will use short class names for common classes
   * in java.lang.* and java.util.*, regardless of {@link #SHORT_TYPE_NAMES}
   * setting.
   */
   ...
   * The net effect is to compute the size of 'obj' by summing over all instance
   * data contained in 'obj' but not in 'base'.
   * 
   * @param base
  ...
  public static int sizedelta(final Object base, final Object obj) {
    ...
    return visited.containsKey(obj) ? 0 : computeSizeof(obj, visited,
  ...
  public static String fieldName(final Field field,
      final boolean shortClassNames) {
    return typeName(field.getDeclaringClass(), shortClassNames).concat("#")
  ...
  private static final class ClassAccessPrivilegedAction implements
  ...
  private static final class FieldAccessPrivilegedAction implements
      PrivilegedExceptionAction {
    public Object run() throws Exception {
  ...
  private static int computeSizeof(Object obj, final IdentityHashMap visited,
    ...
    while (!queue.isEmpty()) {
      ...
      {
        final ClassMetadata metadata = getClassMetadata(objClass, metadataMap,
            caAction, faAction);
  ...
  private static ObjectProfileNode createProfileTree(Object obj,
    ...
    while (!queue.isEmpty()) {
      ...
      if (objClass.isArray()) {
        ...
        final AbstractShellProfileNode shell = new ArrayShellProfileNode(node,
            objClass, arrayLength);
        shell.size = sizeofArrayShell(arrayLength, componentType);
      ...
      } else // the object is of a non-array type
      {
        ...
        shell.size = metadata.m_shellSize;
   ...
   * non-recursive post-order traversal of the tree created in phase 1) and
   * 'locking down' of profile nodes into their most compact form.
   */
  private static void finishProfileTree(ObjectProfileNode node) {
    ...
    while (node != null) {
      ...
      if ((node.size == 1) || (lastFinished == node.m_children[1])) {
        node.finish();
        lastFinished = node;
      } else {
        queue.addFirst(node);
  ...
  private static ClassMetadata getClassMetadata(final Class cls,
      ...
      if (fieldType.isPrimitive()) {
        ...
        if (!field.isAccessible()) {
          try {
            ...
            throw new RuntimeException("could not make field " + field
                + " accessible: " + pae.getException());
          }
        }
    ...
    final ClassMetadata superMetadata = getClassMetadata(cls.getSuperclass(),
   ...
   */
  private static int sizeofArrayShell(final int length,
